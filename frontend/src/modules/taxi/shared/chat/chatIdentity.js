import { getTaxiAdminInfo, getTaxiAdminToken, getTaxiUserToken } from '../authStorage';
...
export const resolveChatRole = (preferredRole) => {
  ...
  if (getTaxiAdminToken()) {
    return 'admin';
  }

  if (readStorageValue('driverToken')) {
    return 'driver';
  }
...
export const resolveChatToken = (preferredRole) => {
  ...
  const adminToken = getTaxiAdminToken();
  const userToken = getTaxiUserToken();
  const driverToken = readStorageValue('driverToken');
...
export const getChatSession = (preferredRole) => {
  ...
  const adminInfo = getTaxiAdminInfo();
