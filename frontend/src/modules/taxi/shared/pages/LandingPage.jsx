...
function LandingPage() {
  ...
  const withPrefix = React.useCallback(
    (path = '') => {
      if (!routePrefix || !path.startsWith('/')) {
        return path;
      }
        ...
        <header className="new-main-header">
          ...
          <div className="new-nav-container">
            ...
            <nav className={`new-nav-links ${isMobileMenuOpen ? 'mobile-open' : ''}`}>
              ...
              <Link to={withPrefix('/about')} className={`new-nav-link ${activeTab === 'about' ? 'active' : ''}`} onClick={() => { setActiveTab('about'); setIsMobileMenuOpen(false); }}>Company</Link>
              <Link to={withPrefix('/services')} className={`new-nav-link ${activeTab === 'services' ? 'active' : ''}`} onClick={() => { setActiveTab('services'); setIsMobileMenuOpen(false); }}>Our Taxi</Link>
              <Link to={withPrefix('/faq')} className={`new-nav-link ${activeTab === 'faq' ? 'active' : ''}`} onClick={() => { setActiveTab('faq'); setIsMobileMenuOpen(false); }}>FAQs</Link>
              <Link to={withPrefix('/blog')} className={`new-nav-link ${activeTab === 'blog' ? 'active' : ''}`} onClick={() => { setActiveTab('blog'); setIsMobileMenuOpen(false); }}>Blog</Link>
      ...
      <footer id="contact" className="new-landing-footer">
        ...
        <div className="footer-main-content">
          ...
          <div className="footer-col-2">
            <h3>Quick Links</h3>
            <ul>
              <li><Link to={withPrefix('/terms')}>Terms & Conditions</Link></li>
              <li><Link to={withPrefix('/privacy')}>Privacy Policy</Link></li>
              <li><Link to={withPrefix('/refund')}>Refund Policy</Link></li>
              <li><Link to={withPrefix('/cancellation')}>Cancellation Policy</Link></li>
              <li><Link to={withPrefix('/contact')}>Contact Us</Link></li>
              <li><Link to={withPrefix('/about')}>About Us</Link></li>
            </ul>
          </div>
          <div className="footer-col-3">
            <h3>Our Services</h3>
            <ul>
              <li><Link to={withPrefix('/services')}>City Rides</Link></li>
              <li><Link to={withPrefix('/services')}>Airport transfers</Link></li>
              <li><Link to={withPrefix('/services')}>Outstation Trips</Link></li>
              <li><Link to={withPrefix('/services')}>Parcel Delivery</Link></li>
              <li><Link to={withPrefix('/services')}>Bike Taxis</Link></li>
              <li><Link to={withPrefix('/contact')}>24/7 Customer Support</Link></li>
            </ul>
          </div>
        </div>
        ...
        <div className="footer-bottom-bar">
          <div className="footer-legal">
            <Link to={withPrefix('/privacy')}>Privacy Policy</Link>
