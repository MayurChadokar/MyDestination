...
export const forgotPassword = async (email) => {
  ...
  await sendEmail({
    to: email,
    subject: `Password Reset OTP for ${process.env.APP_NAME || 'My Desination'}`,
    ...
    html: `
      <div style="font-family: sans-serif; padding: 20px; border: 1px solid #eee; border-radius: 10px; max-width: 500px;">
        ...
        <p style="font-size: 12px; color: #666;">Regards,<br>Team ${process.env.APP_NAME || 'My Desination'}</p>
      </div>
    `,
  });
